// server.js — RastroO + Webhooks Instagram
...
// -------- Banco em memória --------
const DB = { events: [], igEvents: [], comments: {} };
...
function pushRow(map, key, u){ if(!map.has(key)) map.set(key,{key, hits:0, leads:0, sales:0, revenue:0}); u(map.get(key)); }
function summarize(events, from, to){
  const rows=events.filter(e=>inRange(e.ts,from,to));
  let hits=0, leads=0, sales=0, revenue=0;
  const perCreator=new Map(), perVideo=new Map();
  for(const e of rows){
    ...
    pushRow(perCreator, creator, r=>{
      if(e.type==='hit') r.hits++; if(e.type==='lead') r.leads++;
...
(function(){
  ...
  (function(){
    ...
    ['r','utm_source','utm_medium','utm_campaign','utm_term','utm_content','vid','vurl','vh'].forEach(k=>{ if(q[k]){a[k]=q[k]; ch=true;}});
    if(ch) save(a);
  ...
  function send(type,payload){
    ...
    fetch(API+'/api/event',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body),keepalive:type!=='hit'}).catch(function(){});
  }
  window.RastroO={
...
// -------- Dashboard simples (usa /api/report) --------
app.get('/public/dashboard_v2.html', (_req,res)=> res.type('html').send('<!doctype html><meta charset=utf-8><title>RastroO</title><p>OK: <a href=/api/report>report</a> | <a href=/api/ig/last>ig/last</a> | <a href=/public/snippet.js>snippet.js</a>'));

// -------- API de eventos --------
app.post('/api/event', (req,res)=>{
  const { type, creator } = req.body || {};
  if(!type || !['hit','lead','sale'].includes(type)) return res.status(400).json({ok:false,error:'invalid type'});
  let amount=0;
  if (req.body && req.body.amount!=null) amount = parseFloat(String(req.body.amount).replace(',','.')) || 0;
  DB.events.push({
    ts:Date.now(), type, creator:(creator||'—').toString().trim(),
    email:req.body.email, orderId:req.body.orderId,
    amount: type==='sale' ? amount : undefined,
    meta:req.body.meta||{}
  });
  res.json({ok:true,ts:Date.now()});
});
app.get('/api/report', (req,res)=> res.json({ok:true, ...summarize(DB.events, req.query.from, req.query.to)}));
...
app.post('/ig/webhook', (req,res)=>{
  try{
    ...
    // Mapeia comment_id -> media_id se vier no payload
...
// Debug para ver último evento IG
app.get('/api/ig/last', (_req,res)=> res.json({ ok:true, total:DB.igEvents.length, last: DB.igEvents.slice(-1)[0]||null, mapSize:Object.keys(DB.comments).length }));
